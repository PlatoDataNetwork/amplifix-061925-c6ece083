...
import { CheckCircle, TrendingUp, FileText, Users, BarChart3, Target, Lightbulb } from "lucide-react";
...
const Analytics = () => {
  const { data, isLoading, error } = useJsonData<AnalyticsData>('analytics.json');
  ...
  const getIconComponent = (iconName: string) => {
    const icons: Record<string, any> = {
      FileText, TrendingUp, Users, BarChart3, CheckCircle, Target, Lightbulb
    };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="text-center max-w-4xl mx-auto">
          ...
          >
            <Button className="bg-highlight-blue text-white hover:bg-highlight-blue/90">
              {data.analytics.hero.cta_button}
            </Button>
          </a>
        </div>
      </div>
      ...
      <section className="container mx-auto py-16 px-4">
        <h2 className="text-3xl font-bold text-center mb-12">{data.analytics.key_features.title}</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.analytics.key_features.features.map((feature, index) => {
      ...
      <section className="container mx-auto py-16 px-4 bg-accent/10">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12">{data.analytics.benefits.title}</h2>
          
          <div className="space-y-6">
            {data.analytics.benefits.items.map((benefit, index) => (
