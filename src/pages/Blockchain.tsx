...
import { CheckCircle, FileCode, Coins, Wallet, Image, Users, BarChart3 } from "lucide-react";
...
const Blockchain = () => {
  const { data, isLoading, error } = useJsonData<BlockchainData>('blockchain.json');
  ...
  const getIconComponent = (iconName: string) => {
    const icons: Record<string, any> = {
      FileCode, Coins, Wallet, Image, Users, BarChart3, CheckCircle
    };
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="pt-24 container mx-auto py-12 px-4">
        <div className="text-center max-w-4xl mx-auto">
          <h1 className="text-5xl font-bold mb-6 pb-2 bg-gradient-to-r from-blue-500 to-blue-500 bg-clip-text text-transparent">
            {data.blockchain.hero.title} {data.blockchain.hero.title_highlight}
          </h1>
          <p className="text-xl text-muted-foreground mb-8">
            {data.blockchain.hero.description}
          </p>
          <a 
          ...
          >
            <Button className="bg-highlight-blue text-white hover:bg-highlight-blue/90">
              {data.blockchain.hero.cta_button}
            </Button>
          </a>
        </div>
      </div>
      ...
      <section className="container mx-auto py-8 px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {data.blockchain.key_features.features.map((feature, index) => {
      ...
      <section className="container mx-auto py-16 px-4 bg-accent/10">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12">{data.blockchain.benefits.title}</h2>
          
          <div className="space-y-6">
            {data.blockchain.benefits.items.map((benefit, index) => (
